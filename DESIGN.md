...
# PART F — THE DESIGN SYSTEM

`packages/ui-tokens/index.ts` is the single source of visual truth and is **ratified** by this document. This part adds the semantic layer it lacks. **Never hard-code a colour, size or duration** — reference a token.

## F1. Foundations (ratified, unchanged)

| Foundation | Value | Why |
|---|---|---|
| Brand primary | Warm saffron `primary.500 #f97b0a`, ramp 50–950 | Warm, Indian, non-clinical; not the blue of institutional medicine |
| Interactive | Deep teal `teal.500 #2596a3`, ramp 50–950 | Calm, high-contrast against warm neutrals |
| Neutrals | Warm stone `stone.50–950` | Paper, not glass. Never cold blue-grey. |
| Root type size | `18px` (`fontSizes.base`) | Larger than web default by design |
| Touch target | `48px` (`spacing.tap-min`) | WCAG minimum; the person's surface exceeds it (C1.2) |
| Click target | `44px` (`spacing.click-min`) | Desktop |
| Radii | `sm .375 · md .75 · lg 1 · xl 1.5 rem · full` | Soft, not pill-shaped everywhere |
| Motion | `fast 150 · base 250 · slow 400 ms`, ease-out | Short, purposeful |
| Escalation | `L0 stone-500 · L1 teal-500 · L2 blue · L3 amber · L4 orange · L5 red` | The one place colour encodes severity |

> **Contrast note (must be honoured):** `primary.500` on white does **not** meet 4.5:1. Saffron is a **surface, accent and large-display** colour, never body text on light ground. For text-on-light use `primary.700`+; for text-on-saffron use `stone.50`. Enforced by lint rule and checked in review.

## F2. The semantic layer (new)

Components reference **semantic** tokens, never raw ramp values. Raw values appear only in the token file.

### F2.1 Surface and text

```
--surface-page          stone.50      the warm paper ground
--surface-raised        white         cards, sheets
--surface-sunken        stone.100     wells, code, quiet regions
--surface-accent        primary.50    warm highlight regions (rare)
--border-subtle         stone.200
--border-strong         stone.300
--text-primary          stone.900     body — AAA on surface-page
--text-secondary        stone.600     supporting
--text-muted            stone.500     metadata, source chips  (AA at ≥16px only)
--text-inverse          stone.50
--focus-ring            teal.600      3px, 2px offset, never removed
```

### F2.2 Role surfaces

Each role's surface carries a **single, quiet identity cue** — a header rule and accent — so a user always knows which product they are in. Never a full-bleed coloured chrome.

```
--role-person-accent      primary.500    warm, human
--role-caregiver-accent   teal.600       calm, steady
--role-chw-accent         teal.800       deeper, field-serious
--role-clinician-accent   stone.700      restrained, near-neutral
```

The person's surface is warmest; the clinical surface is nearly neutral. Warmth decreases as clinical distance increases.

### F2.3 Evidence semantics (Part D)

```
--evidence-fact-rule        stone.900   solid 3px left rule
--evidence-hypothesis-rule  stone.400   DASHED 3px left rule
--evidence-action-rule      teal.600    solid 3px left rule
--evidence-unknown-rule     stone.300   dotted 3px left rule
--evidence-source-text      text-muted  small caps, letterspaced
```

The **solid/dashed distinction is the load-bearing signal** and must survive greyscale, colour blindness, and a photocopy.

### F2.4 Status and alert semantics

Colour never carries meaning alone; every status pairs colour + glyph + word.
